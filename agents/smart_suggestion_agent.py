...
class SmartSuggestionAgent(BaseAgent):
    ...
    def __init__(self, shared_memory: JurixSharedMemory):
        ...
        # Use same embedding model as retrieval agent
    ...
    def _perceive(self, input_data: Dict[str, Any]) -> None:
        ...
        # Check feedback history for this issue
    ...
    def _analyze_issue_content(self) -> Dict[str, Any]:
        ...
        # Add issue type specific boosts
    ...
    def _suggest_articles(self, search_strategy: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Use enhanced context if available, otherwise build from components
        if enhanced_context:
        ...
        try:
            ...
            # Search for MORE articles initially to get better variety
            initial_search_depth = min(search_depth * 3, 15)  # Get up to 15 articles initially
            ...
            for idx, (doc, meta, distance) in enumerate(zip(
                ...
                # Skip if we've seen this title already (avoid duplicates)
                ...
                # Boost score based on keyword and term matches
                doc_lower = doc.lower()
                ...
                # Only include articles above a minimum threshold
                ...
                article = {
                    ...
                    "distance": distance  # Include for debugging
                }
            ...
            # Apply feedback learning to rerank
    ...
    def _calculate_relevance_score(self, content: str, metadata: Dict[str, Any], 
        ...
        base_score = 1.0 - (rank * 0.1)  # Base score from search rank
    ...
    def _apply_feedback_learning(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # Get historical feedback for this issue type
        issue_type = self.mental_state.get_belief("issue_type")
        type_feedback = self._get_issue_type_feedback(issue_type)
        
        # Re-rank based on historical performance
        for article in articles:
            article_id = article["article_id"]
            
            # Boost score if historically helpful for this issue type
    ...
    def _update_feedback_stats(self, article_id: str, helpful: bool) -> None:
        ...
        self.redis_client.expire(stats_key, 2592000)  # 30 days
